...
```
4. Use `str(fev)` to check to see if all the variables are the correct data types. 
...
str(fev)
```
4a. While we're cleaning the raw data we need to make a few more edits. The FEV1 
    measurements were erroneously recoded in the data set at 100x their real value. 
```{r}
fev[c("FFEV1", "MFEV1", "OCFEV1", "MCFEV1", "YCFEV1")] <- fev[c("FFEV1", "MFEV1", "OCFEV1", "MCFEV1", "YCFEV1")]/100
head(fev$FFEV1)
```

5. Save this data as a tab-delimited text file onto your computer for later use. 
```{r}
write.table(fev, file="C:/Github/MATH456/data/Lung.txt", sep="\t")
```

6. Create a scatterplot of FEV versus height for fathers. 
