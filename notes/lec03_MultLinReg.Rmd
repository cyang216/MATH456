...
* These coefficients are the same as if you standardized the data X and
  Y prior toconducting the regression. 
* The larger the magnitued of the standardized coefficienet, 
  the more $X_{i}$ directly contributes to the prediction of $y$. 
* The standardized slope coefficienet is the amount of change in the mean of
  the standardized $y$ values when the value of $X$ is increaed by one
...
* The covariances $\sigma_{ij}$ are found on the off diagonal entires.  
* This matrix is symmetric

Similarily the correlation matrix can be found using the `cor()` function.
```{r}
round(cor(x.vars), digits=3)
```

* The correlation of a varible with itself is always 1, hence the 1's down the
  diagonal.
...
prediting the values of $y$, we can use an ANOVA. This is the same as testing
...
### Using the standard plots created by R
```{r}
par(mfrow=c(2,2))
plot(mv_model)
```
...
outliers and influential points by examining
...
What if our data didn't come to us in _wide_ format, with separate
variables for mother and father characteristics but in a 
_long_ format where there was a separate variable for gender. 

Here I combine height, FEV and age for both males and females into
a single long data set. 
```{r}
...
What does the relationship between height and FEV look like for
the overall sample? 
```{r}
...
## Stratification
A model where there exists a variable that interacts with all other varibles
...
However to statistically test if they are equivelant some "hand"-calculations
are in order. 
...
* See cautions for simple regression
* Need representative sample
* Violations of assumptions, outlier
...
3. Fit a regression model for mothers using `MFVC` as the dependent 
   variable and age and height as the independent variables. Summarize 
   the results in a tabular form. 
   
4. Test whether the effect of age and height for mothers and fathers are 
