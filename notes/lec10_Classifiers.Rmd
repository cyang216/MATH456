...
* Algorithms may take a while to computer, using `cache=TRUE` in the code chunk
  used to fit the model is advised.
...
```{r}
rValues <- resamples(list(logreg=LogReg,lda1=LDA.1, lda2=LDA.2, nb=NB, 
                          dt=DT, rf=RF, knn=kNN))
par(mfrow=c(1,2))
bwplot(rValues,metric="Sens")            # boxplot
dotplot(rValues,metric="Sens")           # dotplot
...
Recall there are 14/86 (16.3%) cases of diagnosed depression on the testing data set. 

```{r}
confusionMatrix(predict(RF, test), test$cases, positive='Depressed')
```

The Random Forest model had an accuracy of .85 (0.76, 0.92) on the testing data set. 
This model predicted depression correctly 100% of the time, and only 
