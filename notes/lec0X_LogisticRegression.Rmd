---
title: "Lec 0X: Logistic Regression"
...
opts_chunk$set(warning=FALSE, message=FALSE, fig.align='center', fig.height=4, fig.width=5) 
```
...
##  Spam vs Ham
Let's revisit the `email` data set. The `spam` variable will be considered the
outcome variable

* When `spam=1` the email was flagged as spam (an event) 
* When `spam=0` the email was not flagged as spam (non-event)

We will use all 10 other variables in the data set as covariates.
