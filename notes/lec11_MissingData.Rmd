...
We will demonstrate using the Iris data set after we make a certain % of the 
...
```{r}
imp_iris <- mice(iris.mis, m=5, meth="pmm", seed=500, printFlag=FALSE)
summary(imp_iris)
```
...
```
If action is a scalar between 1 and x$m, the function returns the data with imputation number action filled in. Thus, action=1 returns the first completed data set, action=2 returns the second completed data set, and so on
...
or
```{r}
stripplot(imp_iris, pch=16)
```

#### Multivariately 
```{r, fig.width=6, fig.height=6}
xyplot(imp_iris, Sepal.Length ~ Sepal.Width + Petal.Length + Petal.Width | Species ,pch=16)
```
