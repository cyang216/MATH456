...
hiv <- read.delim("~/GitHub/MATH456/data/PARHIV_022216.txt")
...
An F test is then conducted to see if parental FEV adds a signficant amount of 
...
for at least one parent is sigificantly predictive of the FEV1 
...
after this point because it is a change that should be done permenantly to the
...
again, `BSI_interp` also does not rank high amongst the commonly selected
variables. 
...
Adding these two variables provides a significant improvment to the
model. 
```{r}
summary(test_inter)
```
