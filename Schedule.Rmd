...
Navbar: [[Home]](index.html) [[Data]](./data/Datasets.html) [[HW Rubric]](./admin/rubric.html)  [[Google Group]](https://groups.google.com/forum/#!forum/csuc_stat)  [[Google Drive]](https://drive.google.com/a/mail.csuchico.edu/folderview?id=0B83Z8_sNw3KPcnVrYzVFRHUtcHM&usp=sharing)
...
cdat <- cdat %>% select(Week, Day, Date, Topic, raw_notes, Evaluations, link_it)
...
clean <- lcdat %>% mutate(Topic=ifelse(is.na(Topic), '-', Topic), 
               ...
               select(Date, Day, Topic, Notes, Evaluations)
```
